...
void GM6020_MainTask(GM6020_TypeDef* M) {
	...
	if (M->active_channel[3] == 1) {
		...
		M->torque[3] = (int16_t)(M->motor4_rxbuffer[2] << 8 | M->motor4_rxbuffer[3]);
	}
	...
	M->pid.err[3] = M->loc_set[3] - M->angle[3];
	
	M->pid.sum_err[0] += M->pid.err[0];
	M->pid.sum_err[1] += M->pid.err[1];
	M->pid.sum_err[2] += M->pid.err[2];
	M->pid.sum_err[3] += M->pid.err[3];
	
	M->volt[0] = M->pid.kp * M->pid.err[0] + M->pid.ki * M->pid.sum_err[0] + M->pid.kd * (M->pid.err[0] - M->pid.prv_err[0]);
	M->volt[1] = M->pid.kp * M->pid.err[1] + M->pid.ki * M->pid.sum_err[1] + M->pid.kd * (M->pid.err[1] - M->pid.prv_err[1]);
	M->volt[2] = M->pid.kp * M->pid.err[2] + M->pid.ki * M->pid.sum_err[2] + M->pid.kd * (M->pid.err[2] - M->pid.prv_err[2]);
	M->volt[3] = M->pid.kp * M->pid.err[3] + M->pid.ki * M->pid.sum_err[3] + M->pid.kd * (M->pid.err[3] - M->pid.prv_err[3]);
	
	M->pid.prv_err[0] = M->pid.err[0];
	M->pid.prv_err[1] = M->pid.err[1];
	M->pid.prv_err[2] = M->pid.err[2];
	M->pid.prv_err[3] = M->pid.err[3];
